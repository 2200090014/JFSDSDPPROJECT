...
export default function ViewAllDoctors() {
  ...
  const fetchDoctors = async () => {
    try {
      const response = await axios.get('http://localhost:2003/hms/viewalldr'); // Replace with your API URL
  ...
  const deleteDoctor = async (id) => {
    try {
      await axios.delete(`http://localhost:2003/hms/deletedoctor?id=${id}`); // Replace with your delete URL
      fetchDoctors();
