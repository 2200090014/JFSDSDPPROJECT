...
export default function ViewAllPharmacist() {
  ...
  const fetchPharmacists = async () => {
    try {
      const response = await axios.get('http://localhost:2003/hms/viewallpr'); // Replace with your API URL
  ...
  const deletePharmacist = async (id) => {
    try {
      await axios.delete(`http://localhost:2003/hms/deletepharmacist/${id}`); // Replace with your delete URL
      fetchPharmacists();
