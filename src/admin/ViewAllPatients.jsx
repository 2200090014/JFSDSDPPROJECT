...
export default function ViewAllPatients() {
  ...
  const fetchPatients = async () => {
    try {
      const response = await axios.get('http://localhost:2003/hms/viewallpt'); // Replace with your API URL
  ...
  const deletePatient = async (id) => {
    try {
      await axios.delete(`http://localhost:2003/hms/deletepatient/${id}`); // Replace with your delete URL
      fetchPatients();
